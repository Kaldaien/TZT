...
tzt::INI::File::File (const wchar_t* filename)
{
  ...
  if (ret == 0 && fINI != 0) {
    ...
    //TZT_MessageBox (L"Unable to Locate INI File", filename, MB_OK);
    delete [] wszName;
    wszName = nullptr;
    wszData = nullptr;
  }
...
bool
Import_Section (tzt::INI::File::Section& section, wchar_t* buffer, int start, int end)
{
  int key = start;
  for (int k = key; k < end; k++) {
    if (k < end - 1 && buffer [k] == L'=') {
...
void
tzt::INI::File::import (std::wstring import_data)
{
...
std::wstring&
tzt::INI::File::Section::get_value (std::wstring key)
{
...
bool
tzt::INI::File::Section::contains_key (std::wstring key)
{
...
void
tzt::INI::File::Section::add_key_value (std::wstring key, std::wstring value)
{
...
bool
tzt::INI::File::contains_section (std::wstring section)
{
  return sections.find (section) != sections.end ();
}

tzt::INI::File::Section&
tzt::INI::File::get_section (std::wstring section)
{
...
void
tzt::INI::File::write (std::wstring fname)
{
  FILE*   fOut;
  errno_t ret;
...
const std::map <std::wstring, tzt::INI::File::Section>&
tzt::INI::File::get_sections (void)
{
  return sections;
}